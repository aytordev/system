      ...
      hooks = [
        {
          ...
          command =
            ...
            */
            ''
              mkdir -p "$XDG_DATA_HOME/claude-code/context-backups"
              input=$(cat)
              session_id=$(echo "$input" | jq -r '.session_id // "unknown"')

              backup_file="$XDG_DATA_HOME/claude-code/context-backups/compact-$(date +%Y%m%d-%H%M%S).log"
              ...
              if [ -f "$XDG_DATA_HOME/claude-code/audit/pre-tool.jsonl" ]; then
                grep "\"session\":\"$session_id\"" "$XDG_DATA_HOME/claude-code/audit/pre-tool.jsonl" 2>/dev/null | \
