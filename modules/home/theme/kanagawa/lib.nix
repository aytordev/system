...
_: {
  ...
  variantToAppTheme = variant: style: let
    capitalized = {
      wave = "Wave";
      dragon = "Dragon";
      lotus = "Lotus";
  ...
  in
    ...
    then "Kanagawa ${capitalized.${variant}}"
    else if style == "kebab"
    then "kanagawa-${kebab.${variant}}"
    else if style == "underscore"
    then "kanagawa_${kebab.${variant}}"
    else variant;
  ...
  getContrastFg = colors: variant:
    if variant == "lotus"
    then colors.lotus.lotusInk1.hex
