{
  ...
  themeFiles = if hasThemes then builtins.attrNames (builtins.readDir themesDir) else [];
  ...
  themeSymlinks = if cfg.enableThemes && hasThemes && availableThemes != [] then
    builtins.listToAttrs (map mkThemeSymlink availableThemes)
  else {};
...
in {
  options.applications.terminal.emulators.ghostty = {
    ...
    theme = mkOption {
      ...
      default = if availableThemes != [] then "catppuccin-macchiato" else null;
  ...
  config = mkIf cfg.enable (let
    ...
    finalSettings = baseSettings // (
      if cfg.enableThemes && cfg.theme != null then {
        "theme" = "${config.xdg.configHome}/ghostty/themes/${cfg.theme}.conf";
      } else {}
    );
  in {
