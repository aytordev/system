...
let
  ...
  cfg = config.aytordev.applications.terminal.tools.claude-code;
...
in
{
  options.aytordev.applications.terminal.tools.claude-code.permissionProfile = mkOption {
    type = types.enum [
      "conservative"
      "standard"
      "autonomous"
  ...
  config = mkIf cfg.enable {
    applications.claude-code.settings.permissions = {
      allow =
