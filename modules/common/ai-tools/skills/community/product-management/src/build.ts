...
function generateMarkdown(
  ...
  let md = `# Product Management Guidelines\n\n`;
  md += `**Version ${metadata.version}**  \n`;
  md += `${metadata.organization}  \n`;
  md += `${metadata.date}\n\n`;
  md += `> **Note:**  \n`;
  md += `> This document is mainly for agents and LLMs to follow when applying  \n`;
  md += `> product management frameworks. Humans may also find it useful, but  \n`;
  md += `> guidance here is optimized for AI-assisted PM workflows.\n\n`;
  md += `---\n\n`;
  md += `## Abstract\n\n`;
  ...
  try {
    ...
    try {
      ...
      metadata = {
        ...
        abstract:
          "Product management frameworks for discovery, strategy, requirements, validation, planning, and financial analysis.",
      };
    }
