{
  nix-refactor = ''
    ...
    **Actions based on detection:**
    ```
    IF aytordev detected:
        Launch Task with Dotfiles Expert: "Provide comprehensive aytordev patterns for Nix refactoring including library usage, module structure, option namespacing, and coding conventions"
    ELSE:
    ...
    **--fix-options flag:**
    ```
    □ Apply project option namespacing (e.g., aytordev.*)
